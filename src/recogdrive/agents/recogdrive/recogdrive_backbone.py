...
def create_backbone(model_type: str, checkpoint_path: str, device: str = "cuda"):
    """
    Factory function to create VLM backbone.

    Supports both legacy implementation and new VLM registry.

    Args:
        model_type: Type of model ('internvl', 'qwen')
        checkpoint_path: Path to model checkpoint
        device: Device to load model on

    Returns:
        RecogDriveBackbone instance
    """
    # Try to use new VLM registry first
    try:
        from src.recogdrive.vlm import create_vlm
        logger.info(f"Using VLM registry to create {model_type} backbone")
        vlm = create_vlm(model_type, checkpoint_path, device)
        return VLMWrapper(vlm, model_type)
    except ImportError:
        logger.info("VLM registry not available, using legacy backbone")
        return RecogDriveBackbone(model_type, checkpoint_path, device)
    except Exception as e:
        logger.warning(f"VLM registry failed: {e}, using legacy backbone")
        return RecogDriveBackbone(model_type, checkpoint_path, device)


class VLMWrapper(nn.Module):
    """
    Wrapper to adapt new VLM registry to legacy interface.
    """

    def __init__(self, vlm, model_type: str):
        super().__init__()
        self.vlm = vlm
        self.model_type = model_type
        self.model = vlm.model  # For compatibility
        self.tokenizer = vlm.tokenizer
        self.num_image_token = 256

    def forward(self, pixel_values: torch.Tensor, questions: List[str], num_patches_list: List[int]):
        """Forward pass using new VLM interface"""
        output = self.vlm.encode_image(pixel_values, questions)
        return output
